...
from .models import Product, Order,Result, Entry
from .forms import ProductForm, OrderForm,ResultForm, EntryForm
...
def add_res(request):
    if request.method == "POST":
        ...
        idhash.update(idkey.encode('utf-8'))
        id = idhash.hexdigest()
        if not Result.objects.filter(id=id, a=a, b=b, res=res).exists():
               ls = Result.objects.create(id=id, a=a, b=b, res=res)
               #return render(request, 'dashboard/result.html', {'id': ls.id, 'a': ls.a, 'b': ls.b, 'result': ls.res, })
               return HttpResponse("<h1>The results were calculated! Xsum of %s +  %s  is  %s </h1>" % (ls.a, ls.b, ls.res))
        else:
               ls = Result.objects.get(id=id)
               return HttpResponse("<h1> The Record exists in the database! Xsum of %s +  %s  is  %s </h1>" % (ls.a, ls.b, ls.res))
    else:
...
def entry1(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            a = int(request.GET['a'])
            b = int(request.GET['b'])
            table = Entry.objects.create(a=a, b=b)
        return render(request, 'dashboard/entry.html', {'a':a, 'b':b })
    else:
        return HttpResponse('form is not validated')
